...
def test_icml_venue_full_review(mock_llm, tmp_path):
    """End-to-end: ICML venue produces soundness/significance/originality/clarity scores + decision."""
    ...
    yaml_path = tmp_path / "icml_test.yaml"
    yaml_content = """\
name: icml-test
...
jobs:
  ...
  dimensions:
    ...
    strategy:
      matrix:
        dimension: [soundness, significance, originality, clarity]
      max-parallel: 4
    runs-on: local
    steps:
...
"""
    ...
    # Verify 4 ICML dims present
    for dim in ["soundness", "significance", "originality", "clarity"]:
        ...
        assert 1 <= score_json["score"] <= 4  # ICML range
